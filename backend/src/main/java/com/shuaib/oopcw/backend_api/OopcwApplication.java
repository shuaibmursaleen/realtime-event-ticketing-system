...
public class OopcwApplication {
	...
	private static final VendorFunction vendorFunction = VendorFunction.getInstance();
	private static final CustomerFunction customerFunction = CustomerFunction.getInstance();
	...
	public ResponseEntity<?> setConfiguration(@RequestBody HashMap<String, Object> body) {
		if (body.containsKey("load_config"))
			config.loadConfigJson("./src/main/resources/config.json");
		if (body.containsKey("status"))
			config.setRunStatus((boolean) body.get("status"));
		if (body.containsKey("total_tickets"))
			config.setTotalTickets((int) body.get("total_tickets"));
		if (body.containsKey("ticket_release_rate"))
			config.setTicketReleaseRate((int) body.get("ticket_release_rate"));
		if (body.containsKey("customer_retrieval_rate"))
			config.setCustomerRetrievalRate((int) body.get("customer_retrieval_rate"));
		if (body.containsKey("max_ticket_capacity"))
			config.setMaxTicketCapacity((int) body.get("max_ticket_capacity"));
	...
	public ResponseEntity<?> createVendor(@RequestBody HashMap<String, Object> body) {
		...
		if (body.containsKey("tickets_per_release")) {
			...
			return new ResponseEntity<>("Tickets Per Release is needed!", HttpStatus.BAD_REQUEST);

		if (body.containsKey("release_interval")) {
			releaseInterval = (int) body.get("release_interval");
		} else
			return new ResponseEntity<>("Release Interval is needed!", HttpStatus.BAD_REQUEST);
		...
		LogStream.getInstance()
				.addEvent(String.format("Vendor %d has been added to the system successfully", vendor.getVendorId()));
	...
	public ResponseEntity<?> createCustomer(@RequestBody HashMap<String, Object> body) {
		...
		if (body.containsKey("retrieval_interval")) {
			...
			return new ResponseEntity<>("Retrieval Interval is required!", HttpStatus.BAD_REQUEST);
		...
		LogStream.getInstance().addEvent(
				String.format("Customer %d has been added to the system successfully", customer.getCustomerId()));
	...
	public ResponseEntity<?> toggleCustomer(@PathVariable int customerId, @RequestBody HashMap<String, Object> body) {
		if (body.containsKey("status")) {
			if ((boolean) body.get("status")) {
				...
				LogStream.getInstance()
						.addEvent(String.format("Customer %d has been started successfully", customerId));
				return new ResponseEntity<>("Customer started successfully", HttpStatus.OK);
			} else {
				customerFunction.setCustomerStatus(customerId, false);
				LogStream.getInstance()
						.addEvent(String.format("Customer %d has been stopped successfully", customerId));
	...
	public ResponseEntity<?> deleteCustomer(@PathVariable int customerId) {
		if (customerFunction.getCustomers().containsKey(customerId)) {
			...
			LogStream.getInstance().addEvent(String.format("Customer %d has been removed successfully", customerId));
